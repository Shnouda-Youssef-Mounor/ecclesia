...
class AidDetailsScreen extends StatefulWidget {
  final Map<String, dynamic> aid;

  const AidDetailsScreen({super.key, required this.aid});

  @override
  State<AidDetailsScreen> createState() => _AidDetailsScreenState();
}

class _AidDetailsScreenState extends State<AidDetailsScreen> {
  ...
  Future<void> _loadAidDetails() async {
    setState(() => _isLoading = true);
    
    // تحميل الأفراد المستفيدين من هذه المساعدة
    final allIndividuals = await _db.getAllIndividuals();
    final aidRelations = await _db.database.then((db) => db.query('individual_aids', where: 'aid_id = ?', whereArgs: [widget.aid['id']]));
    
    _individuals = aidRelations.map((relation) {
  ...
  Widget build(BuildContext context) {
    ...
    return Directionality(
      ...
      child: Scaffold(
        appBar: AppBar(
          title: Text('تفاصيل المساعدة', style: GoogleFonts.cairo(fontWeight: FontWeight.bold)),
          ...
          children: [
            Row(
              children: [
                ...
                Expanded(
                  child: Text(widget.aid['organization_name'] ?? '', style: GoogleFonts.cairo(fontSize: 24, fontWeight: FontWeight.bold, color: AppColors.primary)),
                ),
              ],
            ),
                      ...
                      Expanded(
                        child: Column(
                          ...
                          children: [
                            ...
                            Text(widget.aid['description']?.toString() ?? 'غير محدد', style: GoogleFonts.cairo(fontSize: 16, color: Colors.black87)),
                          ],
                        ),
                      ),
                    ],
                      ...
                      Expanded(
                        child: Column(
                          ...
                          children: [
                            ...
                            const SizedBox(height: 4),
                            Text(widget.aid['schedule']?.toString() ?? 'غير محدد', style: GoogleFonts.cairo(fontSize: 16, color: Colors.black87)),
                          ],
                        ),
                      ),
                    ],
  ...
  Widget _buildBeneficiariesInfo() {
    return Card(
      elevation: 4,
      child: Padding(
        padding: const EdgeInsets.all(24),
          ...
          children: [
            Row(
              children: [
                ...
                const SizedBox(width: 12),
                Text('المستفيدون (${_individuals.length})', style: GoogleFonts.cairo(fontSize: 20, fontWeight: FontWeight.bold, color: AppColors.primary)),
              ],
            ),
            const SizedBox(height: 16),
              ...
              Container(
                ...
                child: Row(
                  children: [
                    ...
                    const SizedBox(width: 12),
                    Text('لا يوجد مستفيدون من هذه المساعدة', style: GoogleFonts.cairo(fontSize: 16, color: Colors.grey[600])),
                  ],
                ),
              )
            else
